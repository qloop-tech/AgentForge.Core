...
function normalizeWebhook(webhook: Omit<Webhook, 'events'> & { events: string[] | string | null }): Webhook {
  ...
  if (typeof webhook.events === 'string') {
    try {
      return { ...webhook, events: JSON.parse(webhook.events) as string[] };
    } catch {
...
export const webhookApi = {
  listBySession: async (sessionId: string) =>
    (await request<Webhook[]>(`/sessions/${sessionId}/webhooks`)).map(normalizeWebhook),
  listAll: async () => (await request<Webhook[]>('/webhooks')).map(normalizeWebhook),
  get: async (sessionId: string, id: string) =>
    normalizeWebhook(await request<Webhook>(`/sessions/${sessionId}/webhooks/${id}`)),
