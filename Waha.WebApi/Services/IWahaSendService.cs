...
/// </summary>
public interface IWahaSendService
{
    /// <summary>Sends a plain text message. Available in Core and Plus.</summary>
    Task SendTextAsync(string chatId, string text, CancellationToken ct = default);

    /// <summary>
    /// Sends an image to the chat.
    /// <list type="bullet">
    ///   <item><b>Plus (NOWEB)</b>: native image bubble via <c>POST /api/sendImage</c>.</item>
    ///   <item><b>Core</b>: sends the URL as text with <c>linkPreview: true</c> so WhatsApp
    ///   renders a link-preview card — the best available fallback on the free tier.</item>
    /// </list>
    /// </summary>
