...
- Published Compose now exposes the `webhook` service on `WEBHOOK_HOST_PORT` so you can front it with a VPS public IP, reverse proxy, or an external tunnel such as Microsoft Dev Tunnels, ngrok, or Cloudflare Tunnel.
...
3. Export the runtime values the generated `.env` leaves blank:
   ...
   `aspire publish`; `WEBHOOK_HOST_PORT` is the host-side port that exposes the webhook container.
...
to use a **named Cloudflare Tunnel** and deploy one vertical at a time behind a per-vertical hostname
such as `travel-demo.qloop.tech`.
...
- assigns `https://<vertical-id>-demo.qloop.tech` to the named tunnel
...
routes a single demo hostname to the current host-published webhook port.
